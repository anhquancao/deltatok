...
class DinoVisionTransformer(nn.Module):
    ...
    def _get_intermediate_layers_not_chunked(self, x, n=1, export_feat_layers=None, **kwargs):
        ...
        if x.dim() == 5:
            B, S, _, H, W = x.shape
            x = self.prepare_tokens_with_masks(x) # [2, 5, 445, 1024])
        else:
            B, S, _, _ = x.shape
            # Extract H, W from kwargs for pre-computed tokens
            H = kwargs.get('H')
            W = kwargs.get('W')
            if H is None or W is None:
                raise ValueError("H and W must be provided via kwargs when input is pre-computed tokens")
        ...
        for i, blk in enumerate(self.blocks):
            ...
            # Slice recon tokens after the configured layer while keeping
            # the first recon frame as global-coordinate anchor.
            slice_layer_idx = kwargs.get("slice_layer_idx")
            n_gen_views = kwargs.get("n_gen_views")
            if (
                n_gen_views is not None
                and n_gen_views > 0
                and slice_layer_idx is not None
                and i == slice_layer_idx
                and n_gen_views < x.shape[1]
            ):
                # print(f"Slicing recon tokens at layer {i}, {x.shape}")
                x = torch.cat([x[:, :1], x[:, -n_gen_views:]], dim=1)
                if "local_x" in locals():
                    local_x = torch.cat([local_x[:, :1], local_x[:, -n_gen_views:]], dim=1)
                if pos is not None:
                    pos = torch.cat([pos[:, :1], pos[:, -n_gen_views:]], dim=1)
                if pos_nodiff is not None:
                    pos_nodiff = torch.cat([pos_nodiff[:, :1], pos_nodiff[:, -n_gen_views:]], dim=1)
            ...
            if i in export_feat_layers:
                # Export tuple: (processed_feat, raw_state)
                # - processed_feat: normalized, cls/register stripped (backward compatible with da3.py)
                # - raw_state: (x, local_x) with cls/register intact for aux branch
                processed_feat = self.norm(x.clone())[..., 1 + self.num_register_tokens:, :]
                raw_state = (x.clone(), local_x.clone())
                aux_output.append((processed_feat, raw_state))
    ...
    def get_intermediate_layers(
        ...
        **kwargs,
        ...
        # NOTE: aux_outputs are NOT normalized and NOT stripped of cls/register tokens
        # This is intentional to allow aux branch to receive raw token states
    ...
    def _get_intermediate_layers_from_layer(
        self,
        x: torch.Tensor,
        ...
        **kwargs,
        ...
        """Resume backbone forward pass from a cached intermediate layer state.

        Args:
            x: (B, S, N, embed_dim) token state including cls at position 0.
            local_x: (B, S, N, embed_dim) local-attention token state.
            start_layer: block index to resume from (e.g. 19).
            H, W: original image dimensions (needed for RoPE).
            n: which output layers to collect (int or list of indices).
            export_feat_layers: intermediate layers to export raw states.
            **kwargs: ref_view_strategy, attn_mask, etc.

        Returns:
            (output, aux_output) – same format as _get_intermediate_layers_not_chunked.
        """
        if export_feat_layers is None:
            export_feat_layers = []
    ...
    def get_intermediate_layers_from_layer(
        self,
        x: torch.Tensor,
        local_x: torch.Tensor,
        start_layer: int,
        ...
        **kwargs,
        ...
        """Public API to resume from a cached layer state.

        Same post-processing (norm, cat_token, strip cls/register) as
        get_intermediate_layers.
        """
        outputs, aux_outputs = self._get_intermediate_layers_from_layer(
            x, local_x, start_layer, H, W, n,
            export_feat_layers=export_feat_layers, **kwargs
        )
        camera_tokens = [out[0] for out in outputs]
        if outputs[0][1].shape[-1] == self.embed_dim:
            outputs = [self.norm(out[1]) for out in outputs]
    ...
    def get_intermediate_layers_from_tokens(
        self,
        ...
        **kwargs,
        ...
        """
        NOTE: not originally from DA3 
        Process pre-computed patch tokens (from RaymapEncoderDA3) through DinoV2 blocks.
        
        Args:
            patch_tokens: (B, S, N, embed_dim) pre-computed tokens from RaymapEncoderDA3
            H, W: original image dimensions for position encoding
            n: which layers to return (int for last n, or list of indices)
            export_feat_layers: intermediate layers to export
            **kwargs: additional arguments (cam_token, ref_view_strategy, attn_mask)
            
        Returns:
            Same format as get_intermediate_layers
        """
        B, S, N, _ = patch_tokens.shape
        
        # Prepend cls_token and register_tokens to match prepare_tokens_with_masks
        # This ensures RoPE positions align with token sequence
        patch_tokens = rearrange(patch_tokens, "b s n c -> (b s) n c")
        cls_token = self.prepare_cls_token(B, S)
        patch_tokens = torch.cat((cls_token, patch_tokens), dim=1)  # Add cls token
        if self.register_tokens is not None:
            patch_tokens = torch.cat(
                (
                    patch_tokens[:, :1],  # cls token
                    self.register_tokens.expand(patch_tokens.shape[0], -1, -1),  # register tokens
                    patch_tokens[:, 1:],  # patch tokens
                ),
                dim=1,
            )
        patch_tokens = rearrange(patch_tokens, "(b s) n c -> b s n c", b=B, s=S)
        
        outputs, aux_outputs = self._get_intermediate_layers_not_chunked(
            patch_tokens, n, export_feat_layers=export_feat_layers, H=H, W=W, **kwargs
        )
        camera_tokens = [out[0] for out in outputs]
        if outputs[0][1].shape[-1] == self.embed_dim:
            outputs = [self.norm(out[1]) for out in outputs]

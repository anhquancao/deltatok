...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None, max_iter=None):
        ...
        if torch.cuda.is_available():
            log_msg.append('max mem: {memory:.0f} (reserved: {reserved:.0f})')
            ...
            if i % print_freq == 0 or i == len_iterable - 1:
                ...
                if torch.cuda.is_available():
                    print(log_msg.format(
                        ...
                        reserved=torch.cuda.max_memory_reserved() / MB), flush=True)
                else:
                    print(log_msg.format(
                        i, len_iterable, eta=eta_string,
                        meters=str(self),
...
def setup_for_distributed(is_master):
    ...
    def print(*args, **kwargs):
        ...
        # force = force or (get_world_size() > 8)  # Disabled: causes all processes to print in multi-node training
...
def init_distributed_mode_jz(args):
    ...
    # if idr_torch.rank == 0:
    print("Rank {}, local rank {}, >>> Training on {} processes".format(
        idr_torch.rank, idr_torch.local_rank, idr_torch.world_size), flush=True) 
    print("Initializing PyTorch distributed ...", flush=True)
    torch.distributed.init_process_group(backend=args.dist_backend, init_method=args.dist_url,
                                         world_size=idr_torch.size, rank=idr_torch.rank, 
                                         timeout=datetime.timedelta(seconds=3600))
    torch.cuda.set_device(args.gpu)
    torch.distributed.barrier()
    setup_for_distributed(idr_torch.rank == 0)

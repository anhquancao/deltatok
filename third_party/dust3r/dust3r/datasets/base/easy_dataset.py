...
class EasyDataset:
    ...
    def make_sampler(self, batch_size, shuffle=True, world_size=1, rank=0, drop_last=True):
        if not (shuffle):
            raise NotImplementedError("Sampler can work only with Shuffle=True")  # cannot deal yet
        num_of_aspect_ratios = len(self._resolutions) if hasattr(self, '_resolutions') else 1
...
class ResizedDataset (EasyDataset):
    ...
    def set_epoch(self, epoch):
        ...
        if len(self.dataset) == 0:
            raise ZeroDivisionError(f"ResizedDataset error: self.dataset {repr(self.dataset)} has length 0. Cannot resize to {self.new_size}.")
...
class CatDataset_MUSt3R(CatDataset, EasyDataset_MUSt3R):
    ...
    @property
    def ray_map_prob(self):
        return self.datasets[0].ray_map_prob

    @property
    def ray_map_idx(self):
        return self.datasets[0].ray_map_idx
...
class MulDataset_MUSt3R(MulDataset, EasyDataset_MUSt3R):
    ...
    @property
    def ray_map_prob(self):
        return self.dataset.ray_map_prob

...
class PointCloudOptimizer(BasePCOptimizer):
    ...
    def __init__(self, *args, optimize_pp=False, focal_break=20, shared_focal=False, flow_loss_fn='smooth_l1', flow_loss_weight=0.0,
                 ...
                 sintel_ckpt=False, use_self_mask=False, pxl_thre=50, sam2_mask_refine=True, motion_mask_thre=0.35, batchify=True, optimize_scale=True, **kwargs):
    ...
    def get_semantic_2ds_batch(self):
        return self.semantic_2ds

    def get_semantic_2ds_non_batch(self):
        return [s for s in self.semantic_2ds]

    def get_semantic_2ds(self):
        if self.batchify:
            return self.get_semantic_2ds_batch()
        else:
            return self.get_semantic_2ds_non_batch()
    ...
    def forward_batchify(self, epoch=9999):
        # pw_poses = self.get_pw_poses(scale=False)  # cam-to-world
        # monst3r: -0.185, 1.015
        pw_poses = self.get_pw_poses(scale=self.optimize_scale)

        # monst3r: 1
        pw_adapt = self.get_adaptors(scale=self.optimize_scale).unsqueeze(1)
        ...
        # top_weight_i = self._weight_i >= torch.quantile(self._weight_i, 0.5, dim=1, keepdim=True)
        # top_weight_j = self._weight_j >= torch.quantile(self._weight_j, 0.5, dim=1, keepdim=True)
        # # top_weight_i = torch.ones_like(self._weight_i)
        # # top_weight_j = torch.ones_like(self._weight_j)
        # top_weight_i = top_weight_i.float()
        # top_weight_j = top_weight_j.float()
        # print(f'top_weight_i: {top_weight_i.mean()}, top_weight_j: {top_weight_j.mean()}')

        # li = self.dist(proj_pts3d[self._ei], aligned_pred_i, weight=self._weight_i * top_weight_i).sum() / self.total_area_i
        # lj = self.dist(proj_pts3d[self._ej], aligned_pred_j, weight=self._weight_j * top_weight_j).sum() / self.total_area_j
        
        # (Pdb) proj_pts3d[self._ei].min()
        # tensor(-555.7440, device='cuda:0')
        # (Pdb) proj_pts3d[self._ei].max()
        # tensor(580.9030, device='cuda:0')
        # (Pdb) aligned_pred_i.min()
        # tensor(-17.9765, device='cuda:0')
        # (Pdb) aligned_pred_i.max()
        # tensor(52.1047, device='cuda:0')
        li = self.dist(proj_pts3d[self._ei], aligned_pred_i, weight=self._weight_i).sum() / self.total_area_i
        lj = self.dist(proj_pts3d[self._ej], aligned_pred_j, weight=self._weight_j).sum() / self.total_area_j

        # w_temp = self._weight_i.reshape(50, 144, 512)
        # import matplotlib.pyplot as plt
        # plt.imsave('weight_default.png', w_temp[0].cpu().numpy(), cmap='magma')
        ...
        print(f"li: {li}, lj: {lj}, temporal_smoothing_loss: {temporal_smoothing_loss}, flow_loss: {flow_loss}, depth_prior_loss: {depth_prior_loss}")
    ...
    def forward_non_batchify(self, epoch=9999):
        ...
        pw_poses = self.get_pw_poses(scale=self.optimize_scale)  # pair-wise poses (or adaptive poses)
        pw_adapt = self.get_adaptors(scale=self.optimize_scale)

        proj_pts3d = self.get_pts3d()   # 3D point clouds for each image
        weight_i = {i_j: self.conf_trf(c) for i_j, c in self.conf_i.items()}
        weight_j = {i_j: self.conf_trf(c) for i_j, c in self.conf_j.items()}
        for i_j, c in self.conf_i.items():
            print(f'weight_i: {i_j}, {c}')
            print(f'self.conf_i: {self.conf_trf(c)}')
        loss = 0.0

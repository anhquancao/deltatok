...
from occany.fast3r import Fast3r # noqa
...
# from occany.utils.helpers import save_rgb_images
...
def get_args_parser():
    ...
    parser.add_argument('--distill_model', default=None, \
        type=str, help="string containing the distillation model to build")
    ...
    parser.add_argument('--not_pred_raymap', action='store_true', default=False, help='do not use raymap as input to generate image/pointmap')
...
def load_distill_model(args, device):
    ...
    if args.distill_model == 'SAM3':
        ...
        elif args.distill_model == 'SAM2_large':
...
def train(args):
    ...
    # decoder = Must3rDecoder(img_size=(512, 512), enc_embed_dim=1024,
    #                         embed_dim=768,
    #                         feedback_type='single_mlp', memory_mode='kv')
    # decoder = CausalMust3rDecoder(img_size=(512, 512), 
    #     enc_embed_dim=1024, 
    #     embed_dim=768,
    #     pointmaps_activation=ActivationType.LINEAR,
    #     feedback_type='single_mlp', memory_mode="kv", mem_dropout=0.1, 
    #     dropout_mode='temporary', use_xformers_mask=True, use_mem_mask=True)
    ...
    if args.pretrained and last_ckpt_fname is None:
        ...
        if args.pretrained_occany:
            ...
            # if raymap_encoder is not None and raymap_encoder.in_chans == 6:
    ...
    # If using SAM3 distillation, load pretrained SAM3 neck weights into decoder.head_sam.neck and freeze
    if args.distill_model == 'SAM3' and hasattr(decoder, 'head_sam') and hasattr(decoder.head_sam, 'neck'):
        print('Loading pretrained SAM3 neck weights into decoder.head_sam.neck...')
        ...
        # Our SAM3Head neck is at: decoder.head_sam.neck.convs
        sam3_neck_state = {}
        ...
        # Load into our neck
        neck_load_status = decoder.head_sam.neck.load_state_dict(sam3_neck_state, strict=False)
        print('SAM3 neck load_state_dict status:', neck_load_status)
    
        # Freeze the neck parameters
        for param in decoder.head_sam.neck.parameters():
            param.requires_grad = False
        print('Froze decoder.head_sam.neck parameters')

    
    # If predicting raymap, create gen_decoder from decoder and freeze the original decoder
    if not args.not_pred_raymap:
        import copy
    ...
    if args.distributed:
        ...
        # Only wrap decoder with DDP if it's trainable (i.e., not predicting raymap)
        if args.not_pred_raymap:
    ...
    test_criterion_gen = eval(args.test_criterion_gen or args.train_criterion).to(device)
    ...
    # Debug: check if SAM3 neck bias is frozen at optimizer construction time
    ...
    # print("WARNING: just to test drawing")
    # for test_name, testset in data_loader_test.items():
    #     stats = test_one_epoch(img_encoder=img_encoder, raymap_encoder=raymap_encoder, 
    #                             decoder=decoder, criterion=test_criterion, data_loader=testset,
    #                             device=device, epoch=args.start_epoch,
    #                             distill_model=distill_model,
    #                             log_writer=None, args=args, prefix=test_name)
...
def train_one_epoch(img_encoder: torch.nn.Module, 
    ...
    for data_iter_step, batch in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        ...
        # Use loss_of_one_batch_occany_gen when predicting raymap
        if not args.not_pred_raymap:
            batch_result = loss_of_one_batch_occany_gen(views=batch, 
                                                    ...
                                                    dtype=dtype,
                                                    not_pred_raymap=args.not_pred_raymap,
        ...
        # Only run loss_scaler if loss is non-zero
        # if loss != 0:
        #     loss_scaler(loss, optimizer, parameters=model.parameters(), update_grad=(data_iter_step + 1) % accum_iter == 0)
        # else:
        #     print("Loss is 0, skipping loss_scaler")
...
def test_one_epoch(img_encoder: torch.nn.Module, raymap_encoder: torch.nn.Module, 
    ...
    for idx, batch in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        ...
        if misc.is_main_process() and idx %  1 == 0 and n_draw < 40: # 20 recon, 20 gen
            ...
            # saved = {'gen': False, 'recon': False}
            # for pred_key, gt_key in [('img_preds', 'gt_img'), ('raymap_preds', 'gt_raymap')]:
            pred_key, gt_key = "combined_preds", "combined_gt"
            bs = batch_result[pred_key]['rgb'].shape[0]
            for batch_idx in range(bs):
                n_draw += 1
                local_view_idx = 0
                # pred_img = batch_result[pred_key]['rgb'][batch_idx, local_view_idx]  # (H, W, 3)
                # gt_img = batch_result[gt_key][local_view_idx]['img'][batch_idx]  # (3, H, W)
                # frame_id = batch_result[gt_key][local_view_idx]['label'][batch_idx]
                ...
                # gt_pts3d_local = gt_pts3d_local * valid_mask[..., None].float()
                ...
                for i_ray in range(len(is_raymap)):
                    if is_raymap[i_ray]:
                        ...
                        # pred_img[i_ray, :, -side_width:, :] = torch.tensor([255.0, 0.0, 0.0])  # Right border
                        ...
                        # gt_img[i_ray, :, :border_width, :] = torch.tensor([1.0, 0.0, 0.0])  # Left border
                        # gt_img[i_ray, :, -border_width:, :] = torch.tensor([1.0, 0.0, 0.0])  # Right border
                        ...
                        # pred_depth_color[i_ray, :, :border_width, :] = torch.tensor([1.0, 0.0, 0.0])  # Left border
                        # pred_depth_color[i_ray, :, -border_width:, :] = torch.tensor([1.0, 0.0, 0.0])  # Right border
                        ...
                        # gt_depth_color[i_ray, :, :border_width, :] = torch.tensor([1.0, 0.0, 0.0])  # Left border

...
# Fix torchvision compatibility with PyTorch < 2.1 (register_fake added in 2.1)
import torch.library
if not hasattr(torch.library, 'register_fake'):
    if hasattr(torch.library, 'impl_abstract'):
        torch.library.register_fake = torch.library.impl_abstract
    else:
        def _register_fake_noop(op):
            def decorator(fn):
                return fn
            return decorator

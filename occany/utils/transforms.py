...
def _check_input(value, center=1, bound=(0, float("inf")), clip_first_on_zero=True):
    if isinstance(value, (int, float)):
        if value < 0:
            raise ValueError("If  is a single number, it must be non negative.")
        ...
        raise TypeError("should be a single number or a list/tuple with length 2.")
...
def SeqColorJitter(normalize=True):
    """
    Return a color jitter transform with same random parameters.

    When `normalize=True` (default), the returned callable applies jitter then ImgNorm
    (matches the original dust3r behavior). When `normalize=False`, returns a PIL→PIL
    jitter only — useful when downstream normalization differs (e.g. DA3's InputProcessor).
    """
    ...
    def _color_jitter(img):
        ...
        return ImgNorm(img) if normalize else img

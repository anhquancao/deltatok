...
def _ensure_depth_tensor_4d(depth: torch.Tensor, name: str) -> torch.Tensor:
    if depth.ndim == 3:
        return depth.unsqueeze(1)
    if depth.ndim != 4:
        raise ValueError(f"Expected {name} to have shape (B, T, H, W) or (B, H, W), got {tuple(depth.shape)}")
    return depth


def _build_aux_scale_valid_mask(
    gt_flat: torch.Tensor,
    aux_flat: torch.Tensor,
    mask_flat: torch.Tensor | None = None,
    min_depth: float = 0.01,
    max_gt_range: float = 50.0,
) -> torch.Tensor:
    valid = torch.isfinite(gt_flat) & torch.isfinite(aux_flat)
    valid = valid & (aux_flat > min_depth) & (gt_flat > min_depth) & (gt_flat <= max_gt_range)
    if mask_flat is not None:
        valid = valid & (mask_flat > 0)
    return valid


def compute_aux_scale_median(
    gt: torch.Tensor,
    aux: torch.Tensor,
    ...
    aux_flat = aux.reshape(aux.shape[0], -1)
    mask_flat = mask.reshape(mask.shape[0], -1) if mask is not None else None

    valid = _build_aux_scale_valid_mask(
        gt_flat,
        aux_flat,
        mask_flat=mask_flat,
        min_depth=min_depth,
        max_gt_range=max_gt_range,
    )
    ratio = torch.where(valid, gt_flat / (aux_flat + 1e-8), torch.full_like(gt_flat, float('nan')))
    scales = torch.nanmedian(ratio, dim=1).values
    all_invalid = ~valid.any(dim=1)
    scales = torch.where(all_invalid, torch.ones_like(scales), scales)
    scales = scales.clamp(min=1.0 / max_scale, max=max_scale)
    return scales, valid


def compute_aux_scale_ransac(
    gt: torch.Tensor,
    aux: torch.Tensor,
    mask: torch.Tensor | None = None,
    min_depth: float = 0.01,
    ...
    ransac_inlier_thresh: float = 0.1,
    ransac_sample_size: int = 50,
) -> tuple[torch.Tensor, torch.Tensor]:
    B = gt.shape[0]
    device = gt.device
    dtype = gt.dtype
    ...
    aux_flat = aux.reshape(B, -1)
    mask_flat = mask.reshape(B, -1) if mask is not None else None
    ...
    fallback_scales = torch.nanmedian(ratio, dim=-1).values

    has_enough = n_valid >= ransac_sample_size
    if not has_enough.any():
        best_scales = torch.where(torch.isnan(fallback_scales), torch.ones_like(fallback_scales), fallback_scales)
        return best_scales.clamp(min=1.0 / max_scale, max=max_scale), valid

    valid_expanded = valid.unsqueeze(1).expand(-1, ransac_iters, -1)
    rand_scores = torch.rand(B, ransac_iters, gt_flat.shape[1], device=device, dtype=dtype)
    rand_scores = rand_scores.masked_fill(~valid_expanded, -1.0)
    sample_idx = rand_scores.topk(k=ransac_sample_size, dim=-1).indices

    ratio_expanded = ratio.unsqueeze(1).expand(-1, ransac_iters, -1)
    ...
    relative_error = torch.abs(ratio.unsqueeze(1) - candidate_scales.unsqueeze(-1)) / (candidate_scales.unsqueeze(-1) + 1e-8)
    inliers = (relative_error < ransac_inlier_thresh) & valid.unsqueeze(1)
    inlier_counts = inliers.sum(dim=-1)
    inlier_counts = torch.where(torch.isnan(candidate_scales), torch.zeros_like(inlier_counts), inlier_counts)

    best_iter = inlier_counts.argmax(dim=1)
    batch_indices = torch.arange(B, device=device).view(B, 1, 1)
    iter_indices = best_iter.view(B, 1, 1)
    pixel_indices = torch.arange(gt_flat.shape[1], device=device).view(1, 1, -1).expand(B, 1, -1)
    best_inlier_mask = inliers[batch_indices, iter_indices, pixel_indices].squeeze(1)

    inlier_ratios = torch.where(best_inlier_mask, ratio, torch.full_like(ratio, float('nan')))
    best_scales = torch.nanmedian(inlier_ratios, dim=-1).values
    ...
    best_scales = torch.where(torch.isnan(best_scales), torch.ones_like(best_scales), best_scales)
    best_scales = best_scales.clamp(min=1.0 / max_scale, max=max_scale)

    return best_scales, valid


@torch.no_grad()
def build_aux_pseudo_depth(
    aux_depth: torch.Tensor,
    ...
    aux_depth = _ensure_depth_tensor_4d(aux_depth, 'aux_depth')
    gt_depth = _ensure_depth_tensor_4d(gt_depth, 'gt_depth')
    if aux_depth.shape != gt_depth.shape:
        raise ValueError(f"aux_depth and gt_depth must match, got {tuple(aux_depth.shape)} vs {tuple(gt_depth.shape)}")
    if mask is not None:
        mask = _ensure_depth_tensor_4d(mask, 'mask')
        if mask.shape != gt_depth.shape:
            raise ValueError(f"mask and gt_depth must match, got {tuple(mask.shape)} vs {tuple(gt_depth.shape)}")

    if use_ransac:
        scales, scale_valid_mask = compute_aux_scale_ransac(
            gt_depth,
            aux_depth,
            mask=mask,
            min_depth=min_depth,
        ...
        scales, scale_valid_mask = compute_aux_scale_median(
            gt_depth,
            aux_depth,
            mask=mask,
            min_depth=min_depth,
    ...
    pseudo_depth = aux_depth * scales.view(aux_depth.shape[0], 1, 1, 1)
    return pseudo_depth, scales, scale_valid_mask.reshape_as(aux_depth)
...
class ScaleInvariantDepthLoss(nn.Module):
    ...
    def __init__(self, min_depth=0.01, max_depth=50.0, max_scale=100.0, alpha=1.0,
                 l1_weight=1.0, use_ransac=True, ransac_iters=100,
                 ransac_inlier_thresh=0.1, ransac_sample_size=50,
                 ransac_max_pixels=16384):
        super().__init__()
        self.min_depth = min_depth
        self.max_depth = max_depth
        self.max_scale = max_scale
        self.alpha = alpha  # Weight for the gradient loss term
        self.l1_weight = l1_weight  # Weight for the L1 loss term
        ...
        # Cap pixels used for RANSAC to bound (B, ransac_iters, N) memory.
        self.ransac_max_pixels = ransac_max_pixels
    ...
    def compute_scale_median(self, gt, pred, mask=None):
        ...
        valid = (pred_flat > self.min_depth) & (pred_flat < self.max_depth)
    ...
    def compute_scale_ransac(self, gt, pred, mask=None):
        ...
        gt_flat = gt.reshape(B, -1)
        pred_flat = pred.reshape(B, -1)
        N_full = gt_flat.shape[1]

        # Subsample pixels uniformly to bound (B, ransac_iters, N) memory cost.
        # Same indices across the batch is fine — RANSAC just needs an unbiased
        # pixel sample to estimate scale.
        if self.ransac_max_pixels is not None and N_full > self.ransac_max_pixels:
            sub_idx = torch.randperm(N_full, device=device)[:self.ransac_max_pixels]
            gt_flat = gt_flat[:, sub_idx]
            pred_flat = pred_flat[:, sub_idx]
            if mask is not None:
                mask = mask.reshape(B, -1)[:, sub_idx]
        N = gt_flat.shape[1]
    ...
    def forward(self, train_depth, aux_depth, gt_depth, mask=None):
        """
        Args:
        ...
        # Loss mask depends only on the pseudo-target validity and the user mask
        # (NOT on the trainable prediction — otherwise under-predictions get
        # silently excluded and the loss can't push them up).
        loss_mask = (pseudo_gt > self.min_depth) & (pseudo_gt < self.max_depth)
        if mask_flat is not None:

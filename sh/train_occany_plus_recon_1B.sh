...
$CMD \
    --train_dataset="6000 @ WaymoSeqMultiView(ROOT='$SCRATCH/data/waymo_processed', \
        ...
        min_memory_num_views=5, frame_interval=1, max_memory_num_views=20, num_views_per_timestep=5, ray_map_prob=$RAY_MAP_PROB,\
        aug_crop=128, z_far=50, split='train', \
        resolution=[(518, 294), (518, 280), (518, 266), (518, 210), (518, 168)], \
        ...
        seq_pkl_name='seq_surround_temporal_sub5_stride9_all.pkl',\
        min_memory_num_views=6, frame_interval=1, max_memory_num_views=20, num_views_per_timestep=6, ray_map_prob=$RAY_MAP_PROB,
        aug_crop=128, z_far=50, split='train', \
        resolution=[(518, 294), (518, 280), (518, 266), (518, 210), (518, 168)], \
        transform=SeqColorJitter, aug_focal=0.9, shuffle_seq_prob=1.0, distill_model_name='SAM3', base_model='da3') + \
    ...
    --test_dataset="206 @ KittiSeqMultiView(KITTI_PREPROCESSED_ROOT='$SCRATCH/data/kitti_processed', \
        ...
        min_memory_num_views=54, max_memory_num_views=24, num_views_per_timestep=6, \
        z_far=50, split='val', \
    ...
    --aux_branch_layers 6 --scale_inv_depth_loss --lambda_scale_inv_depth 1.0 \
     --lambda_depth 1.0 --lambda_pointmap 1.0 --sam3_use_dpt_proj

# `fine_tune_layers` MUST equal the final `aux_branch_layers` backbone blocks
# so the trainable student tail matches the frozen teacher tail (frozen aux_blocks).

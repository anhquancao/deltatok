...
if [ "$SEQ_MODE" = "surround" ]; then
    ...
    export SEQ_ARGS=""
elif [ "$SEQ_MODE" = "surround_temporal" ]; then
    export CAMERA=surround
...
# Frames saved at 10Hz
    ...
    if [ "$SEQ_MODE" = "surround" ] || [ "$SEQ_MODE" = "surround_temporal" ]; then
...
# Frames saved at 10Hz
    ...
    python dataset_setup/base_make_seq.py --prefix "$SEQ_PREFIX" $SEQ_ARGS --dataset once \
        --camera $CAMERA \
        --seq_mode $SEQ_MODE
# Frames saved at 5Hz — use subsampling_rate=1 for surround_temporal
    ...
    if [ "$SEQ_MODE" = "temporal" ]; then
        ...
        python dataset_setup/base_make_seq.py --prefix "$SEQ_PREFIX" --subsampling_rate 1 --max_stride 9 --dataset occ3d_nuscenes \
            --camera $CAMERA \
            --seq_mode $SEQ_MODE
    else

...
# Training uses two passes per step:
#   - per-image frozen teacher pass (frozen prefix + frozen aux tail/head) for pseudo depth
#   - multi-view student pass (frozen prefix + trainable last `aux_branch_layers` blocks + trainable head) for reconstruction
# `fine_tune_layers` MUST equal the final `aux_branch_layers` backbone blocks (here: 18..23 of DA3-LARGE).

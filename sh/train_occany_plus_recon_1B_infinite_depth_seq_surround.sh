...
export EXP_NAME="occany_plus_recon_1B_infinite_depth_seq_surround"
...
# Mirrors the old occany_plus_recon_1B sampling on the new temporal-surround
# pkl. Forward-seq blocks use num_views_per_timestep=1 (default). Surround
# blocks pin one full timestep of N cameras per sample via
# num_views_per_timestep=N + min_views_per_timestep=N + min_num_timesteps=1
# + no_partial_views=True, and use the patched _get_views (random timestep
# anchor + replacement padding).
$CMD \
    --train_dataset="6000 @ WaymoSeqMultiView(ROOT='$SCRATCH/data/waymo_processed', \
        ...
        min_memory_num_views=5, max_memory_num_views=5, frame_interval=1, ray_map_prob=$RAY_MAP_PROB,\
        num_views_per_timestep=5, min_views_per_timestep=5, min_num_timesteps=1, no_partial_views=True, \
        aug_crop=128, z_far=50, split='train', \
        resolution=[(518, 294), (518, 280), (518, 266), (518, 210), (518, 168)], \
        transform=SeqColorJitter, aug_focal=0.9, distill_model_name='SAM3', base_model='da3', load_infinidepth_pseudo=True) + \
        4000 @ DDADSeqMultiView(DDAD_PREPROCESSED_ROOT='$SCRATCH/data/ddad_processed', \
        seq_pkl_name='seq_surround_temporal_sub5_stride9_all.pkl',\
        min_memory_num_views=6, max_memory_num_views=6, frame_interval=1, ray_map_prob=$RAY_MAP_PROB,
        num_views_per_timestep=6, min_views_per_timestep=6, min_num_timesteps=1, no_partial_views=True, \
        aug_crop=128, z_far=50, split='train', \
        resolution=[(518, 294), (518, 280), (518, 266), (518, 210), (518, 168)], \
        transform=SeqColorJitter, aug_focal=0.9, distill_model_name='SAM3', base_model='da3', load_infinidepth_pseudo=True) + \

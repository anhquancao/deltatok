...
REMOTE_LOG_DIRS=(
  # "/mnt/proj1/eu-25-92/occrae_log"
  "/home/it4i-anhquan/OccAny/slurm/output"
  "/home/it4i-anhquan/OccAny/results"
)
LOCAL_LOG_DIRS=(
  # "${REPO_ROOT}/results"
...
usage() {
  cat <<'EOF'
Usage:
  pull_log_karolina.sh [--dry-run] [--remote HOST] [--src DIR --dst DIR]...

Examples:
  ./pull_log_karolina.sh
  ./pull_log_karolina.sh --dry-run
  ./pull_log_karolina.sh --src /remote/results --dst /local/results

Notes:
  - Pulls logs from Karolina to local using rsync.
  - By default, syncs occrae_log results and slurm/output/.

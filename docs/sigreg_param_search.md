...
- `lejepa/multivariate/slicing.py:104,146` has a `clip_value` knob we do not implement: it zeroes any
  ...
  `docs/sigreg_debug.md`. **Out of scope here** — it is a code change, not a parameter — but it is
...
`docs/sigreg_debug.md`. So this axis buys gradient reproducibility (which was `cos=0.08` between two

#!/usr/bin/env python3
"""Train the OccRAE Image Decoder (MAE-style) on frozen OccRAE 3-level features."""
...
from occrae.img_decoder_trainer import ImgDecoderTrainer  # noqa: E402
...
def get_args_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Optional Hydra-style overrides, e.g. training.batch_size=4",
    )
    ...
    try:
        ...
        with open_dict(cfg):
            if args.occany_recon_ckpt:
                cfg.occrae.weights_path = args.occany_recon_ckpt

            output_dir = cfg.get("output_dir", "results")
            run_root = Path(output_dir).expanduser().resolve() / cfg.exp_name
            cfg.checkpoint_dir = str((run_root / "ckpts").resolve()) + "/"
            cfg.writer_log = str((run_root / "tb_logs").resolve()) + "/"

        trainer = ImgDecoderTrainer(
            args=args,
            cfg=cfg,
            device=device,
            rank=rank,

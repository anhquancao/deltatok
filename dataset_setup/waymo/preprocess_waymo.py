...
def get_parser():
    ...
    parser.add_argument('--waymo_dir', default='data/raw/waymo/training')
    parser.add_argument('--output_dir', default='data/waymo_processed')
    parser.add_argument('--workers', type=int, default=1)
    parser.add_argument('--pid', type=int, default=0, help='Process rank (for distributed jobs)')
    parser.add_argument('--world', type=int, default=1, help='Total number of distributed jobs')

    return parser


def main(waymo_root, output_dir, workers=1, pid=0, world=1):
    processed_sequences = extract_frames(waymo_root, output_dir, workers=workers, pid=pid, world=world)
    make_crops(output_dir, workers=workers, sequences=processed_sequences)
...
def extract_frames(db_root, output_dir, workers=8, pid=0, world=1):
    sequences = _list_sequences(db_root)
    # random.shuffle(sequences)
    # Only process sequences assigned to this process
    sequences = [seq for i, seq in enumerate(sequences) if i % world == pid]
    ...
    # Return only sequences that have calib.json (i.e., processed successfully)
    processed = [seq for seq in sequences if osp.isfile(osp.join(output_dir, seq, 'calib.json'))]
    return processed
...
def extract_frames_one_seq(filename):
    ...
    print(f'>> Processed {len(frames)} frames in {filename}')
    return calib, frames


def make_crops(output_dir, workers=16, sequences=None, **kw):
    tmp_dir = osp.join(output_dir, 'tmp')
    if sequences is None:
        sequences = _list_sequences(tmp_dir)
        # Only process sequences that were processed by extract_frames (i.e., have calib.json)
        sequences = [seq for seq in sequences if osp.isfile(osp.join(tmp_dir, seq, 'calib.json'))]
    print(f"{len(sequences)} sequences to be cropped")
    print(f">> sequences: {sequences}")
...
def crop_one_seq(input_dir, output_dir, seq, resolution=1024):
    ...
    #     print(f'Skipping {seq} as it is already cropped')
    #     return
    os.makedirs(out_dir, exist_ok=True)
    ...
    for cam_idx, cam_info in calib:
        # Skip lidar entries that don't have width/height
        if type(cam_idx) == str and "lidar" in cam_idx:
            continue
        cam_idx = str(cam_idx)
    ...
    for frame in tqdm(frames, leave=False):
        ...
        np.savez_compressed(os.path.join(out_dir, frame + 'npz'),
...
if __name__ == '__main__':
    ...
    print(f"workers: {args.workers}, pid: {args.pid}, world: {args.world}")
    main(args.waymo_dir, args.output_dir, workers=args.workers, pid=args.pid, world=args.world)
